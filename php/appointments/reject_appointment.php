...
try {
    ...
    // Update appointment status and set rejection reason
    $stmt = $conn->prepare("
        UPDATE appointments 
        SET status = 'rejected',
            cancellation_reason = ?,
    ...
    $reason = !empty($data['reason']) ? $data['reason'] : 'Rejected by therapist';
    $stmt->bind_param("sii", $reason, $data['appointmentId'], $therapist_id);
